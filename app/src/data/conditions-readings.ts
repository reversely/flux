...
import { cardinal, type Conditions } from '@/store/conditions';
...
/**
 * Every signal the phone can measure, each read for its consequence. One
 * builder feeds the conditions screen and the home strip, so the two
 * cannot disagree. A missing sensor contributes no row: a wrong reading
 * is worse than a missing one.
 */
export function buildReadings(c: Conditions): Reading[] {
  ...
  if (altitude !== undefined && (altitude !== 0 || c.relativeAltitude !== undefined)) {
    ...
    readings.push({
      icon: 'triangle',
      label: 'elevation',
      value: `${Math.round(altitude)} m (${feet} ft)`,
      means:
        feet > 5000
          ? 'Above 5,000 feet water boils cooler: boil three minutes to purify.'
          : 'Below 5,000 feet a one minute rolling boil purifies water.',
    });
  }
